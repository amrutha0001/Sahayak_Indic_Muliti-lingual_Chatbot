...
# Cascade thresholds
SCORE_CONFIDENT = 0.35   # Tier 1 alone is sufficient above this
SCORE_TRY_WEB   = 0.20   # below this, also try Tier 3
SCORE_HOPELESS  = 0.10   # below this, don't even attempt
...
def answer(query, language="English"):
    ...
    tier1_ok = top_score >= SCORE_CONFIDENT and eng and not _no_info(eng) and len(eng) > 60
    ...
    if not tier1_ok:
        ...
        if txt:
            tier = "url"
            sys2 = _SYS + "\nPrioritize the indexed documents. Only use the official website to supplement if the documents lack a specific detail the user asked about. NEVER replace document information with website information."
            try:
                ...
                if candidate and (not eng or _no_info(eng)):
                    eng = candidate
                    print(f"✅ Tier 2: {len(eng)} chars")
            except Exception as e: print(f"❌ Tier 2 LLM: {e}")

    tier2_ok = eng and not _no_info(eng) and len(eng) > 60 and top_score >= SCORE_TRY_WEB
    ...
    if not tier2_ok:
        ...
        if webs:
            sys3 = _SYS + "\nPrioritize the indexed documents above all else. Only use web results if the documents contain NO relevant information at all. NEVER replace or contradict document information with web results."
